 ...
 * Info:
 *
 * We cannot use @types/mocha because it declares global functions like `describe`, `test` or `it`.
 * Those are exactly the same function as jest declares globally, which results in an unresolvable
 * type clash.
 *
 * Since mocha is only used to get the constants & types in a custom reporter and not for the actual
 * tests, the files from @types/mocha has been copied here and the global declarations have been
 * removed.
 */
